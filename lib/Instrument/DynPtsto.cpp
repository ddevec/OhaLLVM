...
bool InstrDynPtsto::runOnModule(llvm::Module &m) {
  ...
  for (auto &fcn : m) {
    ...
    std::for_each(inst_begin(fcn), inst_end(fcn),
        [&omap, &alloca_list, &ret_list, &malloc_list, &free_list,
         &pointer_list]
    ...
    */
    ...
    if (alloca_list.size() > 0) {
      // First, for calls
      // Get external function
      auto call_fcn = m.getFunction(CallInstName);
      std::vector<llvm::Value *> args;
      // NOTE: This should be the first function (before the alloc instr calls)
    ...
    // NOTE: Must do frees before mallocs for realloc functions (which are both
    // frees and allocs), we need to do free then alloc
    // for frees
...
void InstrDynPtsto::addInitializationCalls(llvm::Module &m) {
  ...
  //   objs...
  {
    ...
    // Do one for Argv
    auto argv = main_fcn->arg_begin();

    if (argv != main_fcn->arg_end()) {
      ++argv;
    }

    if (argv != main_fcn->arg_end()) {
      first_inst = addMallocCall(m, ObjectMap::ArgvValue, &(*argv),
          llvm::ConstantInt::get(i64_type, 16*8), first_inst);
    }
  }
